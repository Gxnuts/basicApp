...
def upload_to_server(file_path):
    ...
    try:
        ...
        print(f"Connection to {SERVER_HOST}:{SERVER_PORT} refused. Make sure the server is running.")
        return
    
    try:
        client_socket.send(f"{file_path}{SEPARATOR}{filesize}".encode())
    except ConnectionResetError:
        print("Connection to server was reset. Make sure the server is running.")
        return
